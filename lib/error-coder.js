...
ErrorCoder.prototype.add = function(errorCode) {
	...
	if(!errorsLog.hasOwnProperty(currentStatus)) {
		...
		errorsLog[currentStatus].errorMessages += (messageDelimiter + msg);
	}

	// clean trailing delimiter
	if(errorsLog[currentStatus].errorMessages.indexOf(messageDelimiter) === 0) {
		errorsLog[currentStatus].errorMessages = errorsLog[currentStatus].errorMessages.slice(1).trim();
	}
	...
	if(errorsLog[currentStatus]) {
		...
		if(response) {
			...
			if(response.status && response.json) {
				// reset error code and error messages when sending http response
				errorsLog[currentStatus].errorCode = errorsLog[currentStatus].errorCode.split(errorDelimiter)[0];
