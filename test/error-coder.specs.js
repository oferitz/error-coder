...
describe('error-coder tests', function() {
    ...
    it('should throw Error when trying to add error code that is not defined in the errors map', function (done) {
      ...
      assert.strictEqual(actual.errorMessages, '<br>' + errMap[500]['01'], 'failed to support adding after changing status - wrong error messages');
			done();
		});
	...
	describe('send method', function () {
      ...
      var EC = new errCoder({errorsMap: errMap});
        ...
        .add(55, 'one', 'three', 'five')
			...
			assert.strictEqual(actual.errorMessages, '<br>meh<br>NICE msg<br>333 msg<br>{"foo":"bar"} msg<br>one two three four five', 'failed to return errors object when no response parameter was sent - wrong errorCode');
			done();
		});
      ...
      EC.send(res);
			...
			assert.strictEqual(resData.errorMessages, '<br>' + errMap[400][15], 'failed to send http response - express.js support - incorrect message');
			done();
		});
		...
		it('should send http response - node.js native support', function (done) {
			var res = new MockRes(function() {
				...
				assert.strictEqual(resData.errorMessages, '<br>' + errMap[400][15], 'failed to send http response - node.js native support - incorrect message');
				done();
			});
